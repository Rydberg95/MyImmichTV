...
- ✓ **TV browse grid (v0.7.0/0.7.1)**: every source (Timeline / Favorites / Albums / Search) now lands in a borderless, edge-to-edge mosaic of square thumbnails instead of going straight to fullscreen. D-pad moves cell-to-cell (←/→/↑/↓; ↑ from the top row opens the source menu, ↓ at the bottom keeps loading more months with 4 rows of lookahead), OK opens the selected photo fullscreen (BACK returns to the grid with the cursor preserved), ▶/⏸ media keys start/stop a slideshow from the selected photo. Column count is configurable from the phone's ⚙ sheet (3–12, default 7) and applies live. The selected photo **pops out of the mosaic** (v0.7.1): it scales up 15% over its neighbors with rounded corners and a soft shadow — `Modifier.zIndex` on the grid item is what lets it draw over later items
...
- ✓ D-pad control: ←/→ flip, ↓ filmstrip, ↑ source menu (Timeline / Favorites / Albums / Remote), OK = info overlay — all in the **fullscreen viewer**; the grid has its own key map (see the v0.7.0 bullet above)
...
**Grid pop-out selection (v0.7.1):** replaced the flat amber selection border with a TV-gallery focus effect: the selected cell scales to 1.15×, rounds its corners (14 dp) and casts a soft shadow (20 dp) — all animated in 180 ms via `graphicsLayer` state reads (no recomposition per frame), so the mosaic stays seamless and only the focused photo lifts. The non-obvious bit: a scaled-up grid item is drawn *under* the items placed after it unless the item declares `Modifier.zIndex(1f)` — without it the pop is silently clipped to its top-left. Verified with strip diffs: 97–99 % of the pixels just above/below the popped cell change vs. the unselected state (image extends over the neighbors), and the old amber-border detector finds zero runs.

**TV browse grid (v0.7.0):** the viewer gained a grid mode — the borderless mosaic every source lands in. Design decisions worth remembering: the single `index` state doubles as the grid cursor and the fullscreen position, so switching modes preserves your place both ways (BACK from fullscreen returns to the same cell). The grid reuses the same lazy per-month paging as the slideshow, with a bigger lookahead (4 rows) since d-pad ↓ walks a row at a time. Key handling lives in the same centralized `onPreviewKeyEvent` as everything else — a `gridShown` branch between the menu and fullscreen branches. The asset-detail fetch (info overlay) is suppressed while `gridShown` — otherwise moving the cursor fired one `GET /api/assets/{id}` per cell. Phone commands (`show`/`next`/`prev`/`slideshow`/`info`) force fullscreen, so the remote behaves exactly as before. Column count is a DataStore pref (3–12, default 7) exposed through the same partial-update `/settings` endpoint as the other prefs. Verified on the box: pixel-measured 7 columns (275 px cells) rising to exactly 5 (384 px) after `POST {"gridColumns":5}` with no restart; cursor tracking verified by locating the amber border cell coordinates across →/↓ presses; OK opened asset index 8 = row 1×7+col 1; BACK restored the grid with the cursor at row 1/col 1; ↑ from the top row opened the menu; menu album selection landed in the album's grid; the SPA ⚙ sheet round-tripped the field (7 → 4 → persisted → restored).
...
**Remote "show" sometimes displayed a random image (v0.7.3):** tapping an asset in an old, not-yet-loaded month on the phone occasionally showed a completely unrelated photo. The viewer addresses assets by positional `index` into the flattened month list, while months load asynchronously. A remote "show" to an old bucket loads that bucket (via `resolvePendingShow`) and sets `index = pos` — correct at that instant — but the position also satisfies the grid's lookahead condition (`pos >= assets.size - lookahead`), which then loads `unloaded.first()`: the *newest* unloaded month, i.e. an intermediate one *newer* than the hint bucket. Its assets insert **before** the cursor in the flattened list, shifting everything down, and `index` now pointed at a random asset from the intermediate month (remoteAsset was already cleared). Normal grid scrolling never hits this because it only loads *older* months, which append *after* the cursor. Fix: `loadMonth` computes where the new month's assets will slot into the flattened list (`precedingLoaded`) and shifts `index` by the loaded month's size whenever the cursor sits at/after the insertion point and no remote override is showing — keeping `assets[index]` anchored to the same asset atomically with the list update (no flicker frame). Guards: skip the shift while `remoteAsset != null` (index is stale during an override and `resolvePendingShow` recomputes it fresh) and while `assets` is empty (initial load). Verified on the box via the remote API: a `show` for June[494] on the old build displayed July[494] within 3 s (July's 1008 assets inserted before the cursor, index stuck at 833 = 339+494 — the "random" image was always the newly-loaded month's asset at the *same in-month offset*); on the fixed build the same tap kept the correct asset through the gap fills (index shifted 833→1841 = +July, then May appended at the end with no shift), `next` landed on June[495], and screenshots taken before/after the gap loads diffed at 0.0%.
...
**TV remote:** in the grid: ←/→/↑/↓ move cell-to-cell · ↑ from top row source menu · OK open fullscreen · BACK (from fullscreen) return to grid. In the fullscreen viewer: ←/→ prev/next · ↓ filmstrip · ↑ source menu · OK info (photos) / play-pause (videos) · ▶/⏸ media keys slideshow on/off · BACK closes overlays, then returns to the grid.
