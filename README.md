...
## Feature status (v0.7.0, 2026-08-24)
...
- ✓ **TV browse grid (v0.7.0)**: every source (Timeline / Favorites / Albums / Search) now lands in a borderless, edge-to-edge mosaic of square thumbnails instead of going straight to fullscreen. D-pad moves cell-to-cell (←/→/↑/↓; ↑ from the top row opens the source menu, ↓ at the bottom keeps loading more months with 4 rows of lookahead), OK opens the selected photo fullscreen (BACK returns to the grid with the cursor preserved), ▶/⏸ media keys start/stop a slideshow from the selected photo. Column count is configurable from the phone's ⚙ sheet (3–12, default 7) and applies live
